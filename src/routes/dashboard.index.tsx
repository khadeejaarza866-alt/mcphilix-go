...
function ActiveOrders() {
  ...
  const stats = useMemo(() => {
    ...
    const completedToday = orders.filter((o) => {
      if (o.status !== "Completed" || !o.deliveryDate) return false;
      try {
  ...
  const submit = () => {
    const finalCustomer = form.customer.trim() || "Unnamed Customer";
    const finalPhone = form.phone.trim() || "N/A";
    const finalLocation = form.location.trim() || "N/A";
    const finalItem = form.item || "Unspecified Item";
    const finalSize = form.size || "Unspecified Size";
    const finalDate = form.deliveryDate ? form.deliveryDate.toISOString() : new Date().toISOString();
    const finalNote = form.note.trim() || undefined;
